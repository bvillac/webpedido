...
function retornaFilaTiendas(c, Grid, TbGtable, op) {
    ...
    strFila += '<td width="100px" style="text-align:left"><input class="form-control" value="' + Grid[c]['ART_DES_COM'] + '"  id="txt_nom_' + Grid[c]['ART_ID'] + '" name="txt_nombreUser" type="text"></td>';
...
function pedidoVentasEnterGrid(valor,control){
    if (valor) {//Si el usuario Presiono Enter= True
         control.value = redondea(control.value, Nprodecimal);
    ...
    $.ajax({
            ...
            success: function (data) {
                if (data.status == "OK") {
                    ...
                    eliminarItemsListas(val,TbGtable);
                    alerMessage();
                    return true;
                } else {
                    $("#messageInfo").html(data.message+buttonAlert); 
