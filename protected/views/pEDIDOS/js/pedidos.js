...
function addPrimerItemFavorito(TbGtable, lista, i) {
    /*Remuevo la Primera fila*/
    $('#' + TbGtable + ' >table >tbody').html("");
    /*Agrego a la Tabla de Detalle*/
    $('#' + TbGtable).append(retornaFilaFavorito(i, lista, TbGtable, true));
}

function addVariosItemFavorito(TbGtable, lista, i) {
    i = (i == -1) ? ($('#' + TbGtable + ' tr').length) - 1 : i;
    $('#' + TbGtable).append(retornaFilaFavorito(i, lista, TbGtable, true));
}

function retornaFilaFavorito(c, Grid, TbGtable, op) {
    var RutaImagenAccion = $('#txth_rutaImg').val();
    var strFila = "";
    var imgCol = '<img class="btn-img" src="' + RutaImagenAccion + '/acciones/delete.png" >';
    strFila += '<td style="display:none; border:none;">' + Grid[c]['ART_ID'] + '</td>';
...
function agregarItemsFavorito(opAccion) {
    var tGrid = 'TbG_Favorito';
    ...
    if ($('#txt_ART_DES_COM').val() != "") {
        ...
        if (opAccion != "edit") {
            //*********   AGREGAR ITEMS *********
            var arr_Grid = new Array();
            if (sessionStorage.dts_favorito) {
                /*Agrego a la Sesion*/
                arr_Grid = JSON.parse(sessionStorage.dts_favorito);
                var size = arr_Grid.length;
                if (size > 0) {
                    //Varios Items
                    if (codigoExiste(nombre, 'ART_DES_COM', sessionStorage.dts_favorito)) {//Verifico si el Codigo Existe  para no Dejar ingresar Repetidos
                        arr_Grid[size] = objFavorito(retornarIndexArray(JSON.parse(sessionStorage.src_buscArticulo), 'ART_DES_COM', valor), JSON.parse(sessionStorage.src_buscArticulo),true);
                        sessionStorage.dts_favorito = JSON.stringify(arr_Grid);
                        addVariosItemFavorito(tGrid, arr_Grid, -1);
                        limpiarTexbox();
                    } else {
                        $("#messageInfo").html('Item ya existe en su lista ' + buttonAlert);
                        alerMessage();
                    ...
                    /*Agrego a la Sesion*/
                    //Primer Items
                    arr_Grid[0] = objFavorito(retornarIndexArray(JSON.parse(sessionStorage.src_buscArticulo), 'ART_DES_COM', valor), JSON.parse(sessionStorage.src_buscArticulo),true);
                    sessionStorage.dts_favorito = JSON.stringify(arr_Grid);
                    addPrimerItemFavorito(tGrid, arr_Grid, 0);
                    limpiarTexbox();
                }
            } else {
                //No existe la Session
...
function objFavorito(c, Grid,condicion) {
    rowGrid = new Object();
    rowGrid.ART_ID = Grid[c]['ART_ID'];
    rowGrid.COD_ART = Grid[c]['COD_ART'];
    rowGrid.ART_DES_COM = Grid[c]['ART_DES_COM'];  
...
function recargarGridFavorito() {
    var tGrid = 'TbG_Favorito';
    if (sessionStorage.dts_favorito) {
        var arr_Grid = JSON.parse(sessionStorage.dts_favorito);
        if (arr_Grid.length > 0) {
            $('#' + tGrid + ' >table >tbody').html("");
            for (var i = 0; i < arr_Grid.length; i++) {
                //$('#'+tGrid+' >table >tbody').append(retornaFilaFavorito(i,arr_Grid,tGrid,true));
                $('#' + tGrid).append(retornaFilaFavorito(i, arr_Grid, tGrid, true));
            }
        }
    }
}

function eliminarItemsListasFavorito(val, TbGtable) {
    var ids = "";
    if (sessionStorage.dts_favorito) {
        var Grid = JSON.parse(sessionStorage.dts_favorito);
        if (Grid.length > 0) {
            //$('#'+TbGtable+' >table >tbody >tr').each(function () {
            $('#' + TbGtable + ' tr').each(function () {
                ...
                if (ids == val) {
                    var array = findAndRemove(Grid, 'ART_ID', ids);
                    sessionStorage.dts_favorito = JSON.stringify(array);
                    //if (count==0){sessionStorage.removeItem('detalleGrid')} 
                    $(this).remove();
                    //alerMessage();
                }
...
function guardarFavoritos(accion) {
    //if (validateForm()) {
        //var ID = (accion == "Update") ? $('#txth_CDOR_ID').val() : 0;
        var cliId=$('#cmb_cliente option:selected').val();
        var link = $('#txth_controlador').val() + "/Savefavorito";
        $.ajax({
            type: 'POST',
            url: link,
            data: {
                "DTS_LISTA": sessionStorage.dts_favorito,
                "ACCION": accion
            },
            success: function (data) {
                if (data.status == "OK") {
                    $("#messageInfo").html(data.message+buttonAlert); 
                    //sessionStorage.removeItem('dts_favorito');
                    alerMessage();
                } else {
                    $("#messageInfo").html(data.message+buttonAlert); 
                    alerMessage();
...
function mostrarlistadofavoritos(){
    ...
    var link=$('#txth_controlador').val()+"/BuscarListadoFavorito";
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url:link,
        ...
        success:function(data){                
            if(data.length > 0){                
                ...
                sessionStorage.removeItem('dts_favorito'); 
                for (var i = 0; i < data.length; i++) {
                    arr_Grid[i] = objFavorito(i,data,false)
                    sessionStorage.dts_favorito = JSON.stringify(arr_Grid);
                    $('#' + tGrid).append(retornaFilaFavorito(i, arr_Grid, tGrid, true));
                }
            }else{
...
function consultarListadoFavorito() {
    var link=$('#txth_controlador').val()+"/ListarFavoritos";
    $.fn.yiiGridView.update('TbG_PEDIDO', {
        type: 'POST',
        url:link,
        data:{
...
function loadDataCreateFavorito() {
    recargarGridFavorito();//Recargar Tiendas.
}
