...
/*  VALIDA LA CLASE EN validation_Gm CADA EVENTO DEL CONTROL data-type  */
$(function() {
    $(".contendor, .contentpop").on("keyup", ".validation_Vs", function(event){
...
//                }
    ...
    $('.contendor, .contentpop').on("keyup", ".validationField_Vs", function(event){
        if($(this).attr('data-required') != undefined  || $(this).attr('data-required') != "true"){
...
function validateType(type,valor,ref){
    ...
    switch (type) { 
        case 'number'://solo numeros
            ...
            if(!result.response){
                result.errorMessage = 'Solo se permiten Números';//objLang.Only_allow_numbers_;
            }
            break;
        case 'alfa'://solo letras
            result.response = validarExpresion(/^([a-zA-ZáéíóúAÉÍÓÚÑñ '])+$/,valor);
            if(!result.response){
                result.errorMessage = 'Solo se permite Enter y letras';//objLang.Only_allowed_to_enter_letters_;
            }
            break; 
        case 'alfanumerico':
            result.response = validarExpresion(/^([a-zA-Z áéíóúAÉÍÓÚÑñ0-9])+$/,valor);
            if(!result.response){
                result.errorMessage = 'Solo se permite enter, letras, y números';//objLang.Only_allowed_to_enter_letters_and_numbers_;
            }
            break;
        case 'cedula':
        ...
        case 'email'://email        
            ...
            if(!result.response){
                result.errorMessage = 'El Email no cumple con el formato establecido';//objLang.The_email_field_does_not_have_a_correct_format_;
            }
            break;
        case 'telefono':
            result.response = validarExpresion(/^(((\d{6,9}[ ]?\/[ ]?)(\d{6,9}[ ]?\/[ ]?)*\d{6,9})|(\d{6,9}))$/,valor); 
            if(!result.response){
                result.errorMessage = 'Número de Teléfono invalido';//objLang.Invalid_phone_number_;
            }
            break;
        case 'telefono2':
            result.response = validarExpresion(/^(((\d{6,10}[ ]?\/[ ]?)(\d{6,10}[ ]?\/[ ]?)*\d{6,10})|(\d{6,10}))$/,valor); 
            if(!result.response){
                result.errorMessage = 'Número de Teléfono invalido';//objLang.Invalid_phone_number_;
            }
            break;
        case 'telefono3':
            result.response = validarExpresion(/^(((\d{6,10}( [a-zA-Z0-9.]{2,4})?( [a-zA-Z0-9.]{2,8})?[ ]?\/[ ]?)(\d{6,10}( [a-zA-Z0-9.]{2,4})?( [a-zA-Z0-9.]{2,8})?[ ]?\/[ ]?)*\d{6,10}( [a-zA-Z0-9.]{2,4})?)( [a-zA-Z0-9.]{2,8})?|(\d{6,10}}?)( [a-zA-Z0-9.]{2,4})?( [a-zA-Z0-9.]{2,8})?)$/,valor); 
            if(!result.response){
                result.errorMessage = 'Número de Teléfono invalido';//objLang.Invalid_phone_number_;
            }
            break;
        case 'dinero':
            ...
            if(!result.response){
                result.errorMessage = 'Solo número y decimales despues del punto';//objLang.Only_numbers_and_decimal_point_for_;
            }
            break;
        case 'fecha':
            result.response = validarExpresion(/^(19|20)\d\d[-](0[1-9]|1[012])[-](0[1-9]|[12][0-9]|3[01])/,valor); // Eduardo Cueva. Cambiar mas adelante 
            if(!result.response){
                result.errorMessage = 'Fecha no tiene el formato correcto yyyy-mm-dd';//objLang.Not_valid__the_format_is_yyyy_mm_dd__for_example_2012_12_24_;
            }
            break;
        default:// all
            result.response = validarExpresion(/^.+$/,valor);
            if(!result.response){
                result.errorMessage = 'El campo no puede estar vacío';//objLang.The_field_must_not_be_empty_;
            }
            break;
    }
...
function validateSize(sizeMax,sizeMin,valor){
    ...
    if(sizeMax && sizeMin){
        if(valor.length > sizeMax || valor.length < sizeMin){
            result.response = false;
            if(sizeMin==sizeMax){
                message = 'Debe introducir sólo caracteres de tamaño Mininimo';//objLang.You_must_enter_only__sizeMin__characters_;
                console.log(message);
                message = message.replace(/\{sizeMin\}/g,sizeMin);
            }
            else{
                message = 'Los caracteres deben tener un rango máximo y minimo';//objLang.The_character_range_is_between__sizeMin__and__sizeMax__;
                message = message.replace(/\{sizeMin\}/g,sizeMin);
        ...
        if(sizeMax){
            if(valor.length > sizeMax){
                result.response = false;
                message = 'Máximo tamaño de Caracteres';//objLang.Maximum__sizeMax__characters_;
                message = message.replace(/\{sizeMax\}/g,sizeMax);
            }
        }else{
            if(sizeMin)
                if(valor.length < sizeMin){
                    result.response = false;
                    message = 'Minimo Tamaño de Caracteres';//objLang.At_least__sizeMin__characters_;
                    message = message.replace(/\{sizeMin\}/g,sizeMin);
                }
        }
    }
